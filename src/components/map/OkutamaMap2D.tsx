...
export default function OkutamaMap2D({ onRequest3D }: OkutamaMap2DProps) {
  ...
  // terrainScale適用前の中心: [-7760.500746543941, 1845.8277086543676, 7760.500746543941]
  // terrainScale適用後の中心: [-77605.00746543941, 18458.277086543676, 77605.00746543941]
  // LakeModel 側で position={[77605.00746543941, 0, -77605.00746543941]} を指定している。
  // したがって、現在の3D空間では地形の中心が原点(0,0,0)に一致しており、
  // バウンディングボックスは原点を中心とした対称な範囲として扱える。
  // terrainScale適用前のサイズ: 15521.001650509425 ユニット
  // terrainScale適用後のサイズ: 155210.01650509425 ユニット（約155.2km）
  const modelBounds = useMemo(() => {
    if (!isDevMode) return null;

    // 地形のバウンディングボックスの半サイズ（x,z方向）
    // terrainScale適用前: 15521.001571798653
    // terrainScale適用後: 155210.01571798653（約155.2km）
    const halfSize = (15521.001571798653 * 10) / 2;
