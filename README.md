...
- [ ] **小河内神社の座標を定義する**（69行目の前提データ）
- [ ] **画面中央位置に十字マークを表示する**（70行目 - シンプルで独立、他の機能の視覚的基準になる）

### 優先度2: 基本動作（アプリ起動時の動作）
- [ ] **アプリ開始時、エリア内の場合、自分のGPS位置を画面の中心とする**（68行目）
- [ ] **アプリ開始時、エリア外であれば、小河内神社を画面の中心とする**（69行目）
- [ ] **エリア内の場合、自分のGPS位置を表示する**（67行目 - エリア判定は既に実装済み）

### 優先度3: 既存機能の拡張（既存コードを活用できるもの）
- [ ] **古地図の透明度を調整できるようにする**（80行目 - 既存のoverlayOpacityを活用、独立した機能）
- [ ] **情報リストの一つを選択でき、その位置に選択情報ピンを表示する**（72行目 - 既存のOkutamaMap2Dを拡張）
  - [ ] 同じ情報を押下することで、選択を解除できる（73行目）

### 優先度4: VR機能の拡張（3Dビューの機能追加）
- [ ] **VRモードで、情報位置を表示する**（74行目 - 既存のLocationBasedObjectsを拡張）
  - [ ] 自分の位置からある一定距離内の情報地点で、画面中心に一番近いもののラベルが表示される（75行目）
- [ ] **VRモードで、情報地点の表示に加え、選択情報ピンが表示される**（76行目 - 選択機能に依存）
  - [ ] ピンに距離（キロメートル表示）を表示（77行目）
  - [ ] ピンに吹き出し形式での情報の名称を表示（78行目）
  - [ ] 選択情報ピンを探しやすいよう、画面外の場合、画面の上下左右に矢印が表示される（79行目）

### 優先度5: 高度な機能（複数の機能に依存、実装が複雑）
- [ ] **VRで立つ位置での高さは、3dモデルのその地点を高さにする**（71行目 - 3Dモデルとの連携が必要）
- [ ] **アプリ開始時に、devモード、本番モードを指定できるようにする**（66行目 - 既にタップ方式で実装済み、UI改善のみ）